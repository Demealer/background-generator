...
while True:
    ...
    try:
        if int(user_input1):
            print(f'You chose: ' + user_input1 + ', please choose another number.')
...
if run_the_game:
    ...
    any_number = range(int(numbers_to_randomize[0]),int(numbers_to_randomize[1]))
    print(numbers_to_randomize)
    print(input(range(int(numbers_to_randomize[0]),int(numbers_to_randomize[1]))))
    the_number = random.choice(any_number)
    #print(f'{the_number} is the number, and it was chosen between this: {numbers_to_randomize}')

while True:
    user_input2 = input(f'You chose {numbers_to_randomize} as the numbers you wanted. So we chose a random number in between those numbers. \n Guess what the number is! ')
    try:
        if int(user_input2):
            ...
            if int(user_input2) >= int(numbers_to_randomize[0]) and int(user_input2) <= int(numbers_to_randomize[1]):
                ...
                print(f'Please type a number between {numbers_to_randomize} only.')
                continue
